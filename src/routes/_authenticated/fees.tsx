...
import { z } from "zod";
import { ArrowLeft, DollarSign, Loader2, Plus, Search } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/fees")({
  head: () => ({ meta: [{ title: "Fees — Infinite EduSuite" }] }),
  beforeLoad: ({ context }) => {
...
function FeesPage() {
  ...
  const fees = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("fee_structures")
        .select("id, name, amount, due_date, is_active, branch_id, created_at")
  ...
  const filtered = fees.data?.filter((f) =>
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <h1 className="mt-3 text-2xl font-semibold tracking-tight">Fees</h1>
        <p className="mt-1 text-sm text-muted-foreground">Manage fee structures for each branch.</p>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-card" style={{ boxShadow: "var(--shadow-sm)" }}>
              {fees.isLoading ? (
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="mt-3 text-sm font-medium">No fee structures yet</p>
                  <p className="text-xs text-muted-foreground">Create your first fee structure on the right.</p>
                </div>
              ) : (
                <table className="w-full text-sm">
                  <thead className="border-b border-border bg-muted/30 text-xs uppercase tracking-wider text-muted-foreground">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left font-medium">Amount (KES)</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {filtered?.map((f) => (
                      <tr key={f.id}>
                        ...
                        <td className="px-4 py-3">KES {Number(f.amount).toLocaleString()}</td>
          ...
          <form onSubmit={onSubmit} className="h-fit rounded-2xl border border-border bg-card p-5" style={{ boxShadow: "var(--shadow-sm)" }}>
            <h2 className="text-sm font-semibold">New fee structure</h2>
            <div className="mt-4 space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input id="name" name="name" required maxLength={120} placeholder="Tuition Fee Term 1" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="amount">Amount (KES)</Label>
                <Input id="amount" name="amount" type="number" min={0} required placeholder="50000" />
              </div>
              <div className="space-y-1.5">
